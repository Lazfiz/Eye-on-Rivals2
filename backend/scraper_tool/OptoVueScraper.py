...
def runNews(log=False) :
    ...
    url = "https://blog.visionix.com/clinical-studies"
    NewsArticles = { 'News' : [] }
    ...
    articleElements = driver.find_elements(By.CLASS_NAME, value="blog__listing-item")

    for element in articleElements:
        articleName = element.find_element(By.CLASS_NAME, value="post__title")
    ...
    for article in NewsArticles['News']:
        ...
        postBody = driver.find_element(By.CLASS_NAME, value="post__body")
        dateString = datetime.strptime(postBody.find_element(By.CSS_SELECTOR, 'time').text, "%b %d, %Y %H:%M:%S %p")
...
def runJobs(log=False):
    jobsUrl = "https://www.linkedin.com/jobs/search/?currentJobId=4306823335&f_C=18006916&geoId=92000000&origin=JOB_SEARCH_PAGE_JOB_FILTER"
